...
# 0 = basic features (no text features), 1 = with text features, 2 = webgazer fixation data
DATA_TO_EXTRACT = 2  # Change this as needed
...
for i in range(10):
    for row_i, row in data_filtered.iterrows():
        ...
        if DATA_TO_EXTRACT in [0, 1]:
            ...
            if DATA_TO_EXTRACT == 0:  # No text features
                data_to_append = np.hstack(( [
                                            ...
                                            data_to_append))
            elif DATA_TO_EXTRACT == 1:  # With text features
                data_to_append = np.hstack(( [
                                            worker_id,
                                            trial_name,
                                            in_span_mean,
        ...
        # Trials >= 5 are Information Seeking
        else:
            data_for_cls_is.append(data_to_append)

data_for_cls_nr = np.array(data_for_cls_nr, dtype=object)  # Use dtype=object for mixed types
data_for_cls_is = np.array(data_for_cls_is, dtype=object)

data_for_classifier = np.array(data_for_cls_nr)

# Convert to DataFrame and save with appropriate columns
if DATA_TO_EXTRACT == 0:
    columns = ['id', 'trial_name', 'in_span_mean', 'out_span_mean', 'relative_TRT', 'Text_TRT', 'Target_TRT', 'Fix_Target', 'target_to_fixation_ratio', 'Fix_Total', 'Correct_Flag']
    filename = 'webq_meco_EN_nr-feats.csv'
elif DATA_TO_EXTRACT == 1:
    columns = ['id', 'trial_name', 'in_span_mean', 'out_span_mean', 'relative_TRT', 'token_count', 'token_avg_length', 'Text_TRT', 'Target_TRT', 'Fix_Target', 'target_to_fixation_ratio', 'Fix_Total', 'Correct_Flag']
    filename = 'webq_meco_EN_nr_wtext-feats.csv'
elif DATA_TO_EXTRACT == 2:
    columns = ['id', 'trial_name', 'Correct_Flag', 'webgazer_filtered_data']
    filename = 'webq_meco_EN_nr_webgazer.csv'
